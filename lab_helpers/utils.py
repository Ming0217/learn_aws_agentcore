"""
lab_helpers/utils.py - Shared AWS Utility Functions
=====================================================
This module provides reusable helper functions used across the workshop labs.
It was originally part of the AWS workshop's pre-built lab environment and has
been retained here as-is, with learning notes added.

WHAT'S IN HERE:
  - SSM Parameter Store helpers (get/put/delete)
  - AWS account/region helpers
  - Cognito user pool management (used in later labs for auth)
  - Secrets Manager helpers
  - IAM role creation for AgentCore runtime execution
  - Cleanup utilities for all provisioned resources

LEARNING NOTE: In a real project, you'd split this into focused modules
(e.g., auth.py, iam.py, cleanup.py). It's consolidated here for lab convenience.
"""
import base64
import hashlib
import hmac
...
# Resolve region once at module load time — reused across all functions
REGION = boto3.session.Session().region_name

# Default test user for Cognito pools created in later labs
username = "testuser"
sm_name = "customer_support_agent"

# IAM role/policy names follow a region-scoped naming convention to avoid
# conflicts when the same lab is run in multiple regions
...
def get_ssm_parameter(name: str, with_decryption: bool = True) -> str:
    ...
    LEARNING NOTE: with_decryption=True is required for SecureString parameters
    (encrypted values like secrets). For plain String parameters it's a no-op.
    Always use True as the default to handle both types transparently.
    """
...
def put_ssm_parameter(
    ...
    LEARNING NOTE: Overwrite=True makes this idempotent — safe to call multiple
    times without failing if the parameter already exists. Use with_encryption=True
    to store sensitive values as SecureString (encrypted with KMS).
    """
    ssm = boto3.client("ssm")
    put_params = {
        "Name": name,
...
def get_or_create_cognito_pool(refresh_token=False):
    """
    Creates a Cognito User Pool and App Client for authenticating AgentCore Gateway requests.

    LEARNING NOTE: This is used in later labs when the agent is deployed behind
    an AgentCore Gateway that requires JWT authentication. Cognito acts as the
    identity provider — it issues access tokens that the gateway validates.

    The SECRET_HASH is required when the Cognito App Client has a client secret.
    It's computed as HMAC-SHA256(username + client_id, client_secret) and must be
    included in auth requests alongside the username/password.

    refresh_token=True re-authenticates and returns a fresh bearer token,
    useful when the existing token has expired.
    """
    boto_session = Session()
    region = boto_session.region_name
...
def create_agentcore_runtime_execution_role():
    """
    Creates the IAM role and policy required for AgentCore Runtime to execute the agent.

    LEARNING NOTE: AgentCore Runtime runs your agent as a managed container.
    It needs an IAM execution role (similar to a Lambda execution role) that grants
    permissions to call Bedrock, read SSM, write CloudWatch logs, access ECR images, etc.

    The trust policy allows bedrock-agentcore.amazonaws.com to assume this role,
    scoped to your account and region via Condition keys — this is the least-privilege
    approach for service-to-service trust relationships.

    The policy covers:
      - ECR: pull the agent container image
      - CloudWatch Logs: write execution logs
      - X-Ray: distributed tracing
      - Bedrock: invoke models and retrieve from Knowledge Base
      - AgentCore Memory: read/write memory records
      - SSM: read parameter store values (e.g., memory_id)
      - AgentCore Gateway: invoke gateway endpoints (used in later labs)
    """
...
def agentcore_memory_cleanup(memory_id: str = None):
    """
    Deletes AgentCore Memory stores.

    LEARNING NOTE: If memory_id is provided, deletes that specific store.
    Otherwise, lists and deletes ALL memory stores in the account/region.
    Use the targeted form (with memory_id) in production to avoid accidental
    deletion of unrelated memory stores.
    """
